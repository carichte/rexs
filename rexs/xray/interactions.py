...
_DB_NAME = "elementdata.sqlite"
_DB_PATH = os.path.join(os.path.dirname(__file__), _DB_NAME)
supported_tables = ["BrennanCowan", "Chantler", "CromerLiberman", "EPDL97", 
                    "Henke", "Sasaki", "Windt", "deltaf"]
...
def get_element(element, database = _DB_PATH):
    dbi = sqlite3.connect(database)
    cur = dbi.cursor()
    try:
        Z = int(element)
...
def store_rho_to_db(material, rho, database = _DB_PATH):
    dbi = sqlite3.connect(database)
    cur = dbi.cursor()
...
def get_rho_from_db(material, database = _DB_PATH):
    dbi = sqlite3.connect(database)
    cur = dbi.cursor()
    ...
    dbi.close()
    if not result:
        raise ValueError("Density of material '" + material + "' not found in database."+os.linesep)
    else:
        rho=float(result[0])
    return rho


def get_optical_constants(densities, materials, energy, database = _DB_PATH, table = "Henke", feff = None):
    """
    ...
    if hasattr(densities, "__iter__") and hasattr(materials, "__iter__"):
        ...
        for i in range(len(elements)):
            ...
            if elements[i] in feff:
                ...
                f1, f2 = np.array(get_f1f2_from_db(elements[i], energy, 
...
def get_attenuation_length(composition, energy, density=1, database=_DB_PATH, 
